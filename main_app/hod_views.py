...
def delete_staff(request, staff_id):
    custom_user = get_object_or_404(CustomUser, id=staff_id)

    try:
        with transaction.atomic():
            staff = Staff.objects.filter(admin=custom_user).first()

            if staff:
                # Retrieve the associated Course object
                course = staff.course

                # Try to delete the Staff object
                try:
                    staff.delete()
                except ProtectedError as e:
                    messages.error(
                        request,
                        f"Unable to delete staff. There are dependencies associated with this staff: {e}",
                    )
                    return redirect(reverse("manage_staff"))

                # Check if the associated Course exists
                if course:
                    # Try to delete the Course object
                    try:
                        course.delete()
                    except ProtectedError as e:
                        messages.error(
                            request,
                            f"Unable to delete the associated course. There are dependencies associated with this course: {e}",
                        )
                        return redirect(reverse("manage_staff"))

            # Finally, delete the CustomUser object
            custom_user.delete()

            messages.success(
                request, "Staff and associated dependencies deleted successfully!"
            )
            return redirect(reverse("manage_staff"))
        ...
        messages.error(
            request,
            f"Unable to delete staff and associated dependencies. IntegrityError: {e}",
        )
        return redirect(reverse("manage_staff"))
